...
static NSString* kKeyBundleDisplayNameiTunesArtwork = @"bundleDisplayName";
static NSString* kKeyPrefsBundleDisplayNameChange = @"keyBundleDisplayNameChange";
...
- (IBAction)resign:(id)sender
{
    ...
    [defaults setValue:[bundleDisplayNameField stringValue] forKey:kKeyPrefsBundleDisplayNameChange];
...
- (void)checkUnzip:(NSTimer*)timer
{
    if ([unzipTask isRunning] == 0) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:[workingPath stringByAppendingPathComponent:@"Payload"]]) {
            ...
            if (changeBundleDisplayNameCheckbox.state == NSOnState) {
                [self doBundleDispalyNameChange:bundleDisplayNameField.stringValue];
            }
...
- (BOOL)doBundleDispalyNameChange:(NSString*)newBundleDisplayName
{
    BOOL success = YES;

    success &= [self doAppBundleDispalyNameChange:newBundleDisplayName];
    success &= [self doITunesMetadataBundleDispalyNameChange:newBundleDisplayName];

    return success;
}

- (BOOL)doITunesMetadataBundleDispalyNameChange:(NSString*)newBundleDisplayName
{
    NSArray* dirContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:workingPath error:nil];
    NSString* infoPlistPath = nil;
    ...
    return [self changeBundleDispalyNameForFile:infoPlistPath bundleDispalyNameKey:kKeyBundleDisplayNameiTunesArtwork newBundleDispalyName:newBundleDisplayName plistOutOptions:NSPropertyListBinaryFormat_v1_0];
}

- (BOOL)doAppBundleDispalyNameChange:(NSString*)newBundleDisplayName
{
    NSArray* dirContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[workingPath stringByAppendingPathComponent:kPayloadDirName] error:nil];
    NSString* infoPlistPath = nil;
    ...
    return [self changeBundleDispalyNameForFile:infoPlistPath bundleDispalyNameKey:kKeyBundleDisplayNamePlistApp newBundleDispalyName:newBundleDisplayName plistOutOptions:NSPropertyListBinaryFormat_v1_0];
}

- (BOOL)changeBundleDispalyNameForFile:(NSString*)filePath bundleDispalyNameKey:(NSString*)bundleDispalyNameKey newBundleDispalyName:(NSString*)newBundleDispalyName plistOutOptions:(NSPropertyListWriteOptions)options
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        plist = [[NSMutableDictionary alloc] initWithContentsOfFile:filePath];
        [plist setObject:newBundleDispalyName forKey:bundleDispalyNameKey];
...
- (void)doEntitlementsEdit
{
    ...
    NSString* anotherpath = @"/Users/Eason/Downloads";
    anotherpath = [anotherpath stringByAppendingPathComponent:@"entitlements.plist"];

    NSLog(@"entitlements file path %@", filePath);
    ...
    [xmlData writeToFile:anotherpath atomically:YES];
...
- (void)checkZip:(NSTimer*)timer
{
    if ([zipTask isRunning] == 0) {
        ...
        [entitlementField setStringValue:@""];
...
- (IBAction)changeBundleDisplayNamePressed:(id)sender
{
    if (sender != changeBundleDisplayNameCheckbox) {
        return;
    }

    bundleDisplayNameField.enabled = changeBundleDisplayNameCheckbox.state == NSOnState;
}
...
- (void)disableControls
{
    ...
    [bundleDisplayNameField setEnabled:NO];
    [changeBundleDisplayNameCheckbox setEnabled:NO];
}
...
- (void)enableControls
{
    ...
    [changeBundleDisplayNameCheckbox setEnabled:YES];
    [bundleDisplayNameField setEnabled:changeBundleDisplayNameCheckbox.state == NSOnState];
}
